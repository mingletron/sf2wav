...
fn write_wav<W: Write>(
    ...
    writer.write_all(&1u16.to_le_bytes())?; // num channels (mono)
...
fn sanitize_filename(name: &str) -> String {
    ...
    // Only remove from the end, and only remove once
    let lower = sanitized.to_lowercase();
    if lower.ends_with(".wav") {
        sanitized[..sanitized.len() - 4].to_string()
    } else {
        sanitized
    }
}
...
fn extract_samples(
    ...
    for (i, header) in sample_headers.iter().enumerate() {
        ...
        write_wav(&mut output_file, sample_slice, header.sample_rate)?;

        extracted_files.push(filename.clone());
        println!(
            "Extracted: {} ({} Hz, {} samples)",
            filename,
            header.sample_rate,
            sample_slice.len()
        );
...
mod tests {
    ...
    fn test_write_wav() {
        ...
        write_wav(&mut output, &samples, 44100).unwrap();
